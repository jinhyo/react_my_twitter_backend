...
router.get("/:userId/tweets", async (req, res, next) => {
  ...
  try {
    const tweets = await getSpecificUsersTweets(userId);
...
router.get("/:userId/comments", async (req, res, next) => {
  ...
  try {
    const comments = await getSpecificUsersComments(userId);
...
router.get("/:userId/medias", async (req, res, next) => {
  ...
  try {
    const mediaTweets = await getSpecificUsersMedias(userId);
