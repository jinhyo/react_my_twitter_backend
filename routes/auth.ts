...
import { getUserWithFullAttributes } from "../lib/utils";
...
router.post("/login", isNotLoggedIn, (req, res, next) => {
  passport.authenticate(
    "local",
    (serverError: Error, user: User, errorInfo: { message: string }) => {
      if (serverError) {
      ...
      // 로그인 실행
      return req.login(user, async (passportLoginError: Error) => {
        if (passportLoginError) {
          console.error(passportLoginError);
          return next(passportLoginError);
        }
